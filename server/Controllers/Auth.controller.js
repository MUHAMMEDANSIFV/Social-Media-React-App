...
export const sendotp = async (req, res) => {
  try {
    ...
    res.json({error:'This Email is not found Please try a valid email address'})
  }
  } catch (err) {
    console.log(err);
  }  
...
export const forgottenpassword = async () => {
  try{
    ...
    const password_reset = await userSchema.findOneAndUpdate({email:email},{
      $set:{
         password:password
      }
    })
    ...
    res.json({success:true})
      ...
      res.json({error:'password changing proccess is failed'})
    }
  }else{
